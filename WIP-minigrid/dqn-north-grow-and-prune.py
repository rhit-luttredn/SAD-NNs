...
import tyro
from neurops import NORTH_score, svd_score
                    ...
                    for i in range(len(target_network.growth_net)-1):
                        ...
                        if args.prune:
                            ...
                            if args.prune_matrix == 1:
                                ...
                                pmatrix = target_network.growth_net.activations[str(i)]
                            scores = svd_score(pmatrix)
                            to_prune = np.argsort(scores.detach().cpu().numpy())[:int(0.25*len(scores))]
                            # print(to_prune)
                            print("Layer {}, neurons to prune: {}".format(i, len(to_prune)))
                            target_network.growth_net.prune(i, to_prune, optimizer=optimizer, clear_activations=False)
                            q_network.growth_net.prune(i, to_prune, optimizer=optimizer, clear_activations=False)
                        ...
                        if args.growth:
                            ...
                            if args.growth_matrix == 1:
                                ...
                                # activation matrix of dim 2#neurons x #neurons
                                gmatrix = target_network.growth_net.activations[str(i)]
